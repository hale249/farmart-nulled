...
class AffiliateProSeeder extends BaseSeeder
{
    ...
    protected function seedAffiliates(): void
    {
        ...
        foreach ($customers as $index => $customer) {
            ...
            $totalWithdrawn = $status === BaseStatusEnum::PUBLISHED ?
    ...
    {
        $affiliates = Affiliate::query()->where('status', BaseStatusEnum::PUBLISHED)->get();
        ...
        for ($i = 0; $i < $dateCount; $i++) {
            // More recent dates have higher weights
            $weight = 1 + ($i / $dateCount) * 10;
            $dateWeights[] = $weight;
        }
